...
from utils.db_utils import DbTypes, DbUtils
from utils.file_utils import delete_files, ensure_path, zip_files
from utils.keegen import gen_passphrase, gen_utf8
...
def _process_data_table():
  ...
  with st.form(key="df_form"):
    ...
    st.form_submit_button(
      label="Submit",
      on_click=DbUtils.save_dataframe,
...
def _create_sample_entries(kp_db: PyKeePass):
  for entry in st.secrets["entries"]:
    ...
    url: str = entry["url"]
    kp_db.add_entry(destination_group=kp_db.root_group, url=url, title=title, username=username, password=password)
  kp_db.save()
      ...
      if db_path.exists():
        ...
        if create_row.button(label="Generate Database", key="gen_db"):
          ...
          kp_db: PyKeePass = create_database(
            filename=db_path, keyfile=key_path, password=token_path.read_text(encoding="utf-8")
          )
...
def main():
  ...
  with st.sidebar:
    ...
    st.download_button(
      ...
      data=_download_action(),
    )

  if all_exist:
    try:
      DbUtils.validate_tk_store(DbTypes.KP, db_path, token=token_path, key=key_path)
      _process_data_table()

    ...
    match compression_type:
        case CompressionTypes.ZSTD:
            cctx = zstd.ZstdCompressor(
                level=3, write_checksum=True, write_content_size=True, threads=-1
            )
            with open(f"{src_dir_pth}.tar", "rb") as tar_file, open(
                out_file_pth, "xb"
            ) as zstd_file:
    ...
    match compression_type:
        case CompressionTypes.ZSTD:
            ...
            with tarfile.open(mode="r", fileobj=temp_out_stream) as tar_file:
                members_to_extract = [
                    m for m in tar_file.getmembers() if m.name in ALLOWED_MEMBERS
                ]

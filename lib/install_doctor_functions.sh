...
function ensureTaskInstalled() {
  ...
  if ! type task &> /dev/null; then
    ...
    if [ -f "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/bodega-update-check" ]; then
      TASK_UPDATE_TIME="$(cat "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/bodega-update-check")"
    else
      TASK_UPDATE_TIME="$(date +%s)"
      echo "$TASK_UPDATE_TIME" > "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/bodega-update-check"
    fi
    ...
    if [ "$TIME_DIFF" -gt 900 ] || [ "$TIME_DIFF" -lt 5 ]; then
      date +%s > "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/bodega-update-check"
...
function ensureTaskfiles() {
  ...
  if [ -z "$ENSURED_TASKFILES" ]; then
    ...
    if [ -f "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/ensure-taskfiles" ]; then
      ...
      TASK_UPDATE_TIME="$(date +%s)"
      echo "$TASK_UPDATE_TIME" > "${XDG_CACHE_HOME:-$HOME/.cache}/${_username}/start.sh/ensure-taskfiles"
    fi
    # shellcheck disable=SC2004
    TIME_DIFF="$(($(date +%s) - $TASK_UPDATE_TIME))"

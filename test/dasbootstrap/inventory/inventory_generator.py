...
def inventory_generator(my_network,host_prefix,inventory_file_path):
    ...
    with open(inventory_file_path, 'w') as f:
        for each_ip in IPNetwork(my_network).iter_hosts():
            ...
            instance_final_name = host_prefix + '-' + instance_name1 + '-' + instance_name2 + '-' + str(instance_number_suffix) + '-' + instance_zone_suffix + '\n'
...
if __name__ == "__main__":
    ...
    parser.add_argument("-P", "--path", help="Path of the inventory file. Defaults to: ./my_ansible_hosts", default='my_ansible_hosts')

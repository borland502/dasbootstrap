...
declare -r VENV_PYTHON="{'ansible_python_interpreter': '/usr/local/pyenv/versions/ansible/bin/python3'}"
...
function _setup_or_upgrade_lxc(){
  ...
  # Give time for the ole proxmox to spin up lxc before proceeding - supply system python until venv
  ansible "${APP_NAME}" -m "${ABW_Connection}" -a "connect_timeout=5 delay=10 sleep=5 timeout=120" -i "${A_HOSTS}" -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" -e "${SYS_PYTHON}" --user root || exit 5
...
function _setup_docker(){
  ...
  ansible "${APP_NAME}" -m "${ABI_ROLE}" -a 'name=robertdebock.roles.ca' -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" --user ansible --become 
...
function setup_artifactory_oss_cmd(){
  local APP_NAME=${1}
  setup_generic_cmd ${1}
...
# function setup_gitbackup_cmd(){

# }

function setup_gitea_cmd(){
  local APP_NAME=${1}
  setup_generic_cmd ${1}
...
function setup_generic_docker_cmd(){
  local APP_NAME=${1}
  setup_generic_cmd ${1}
  _setup_docker ${1}
}
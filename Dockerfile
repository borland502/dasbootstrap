...
ENV VIRTUAL_ENV /opt/dasbootstrap-env
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/dasbootstrap/
...
COPY --chown=user:user dasbootstrap .

# Expose the app.
ENTRYPOINT ["/opt/dasbootstrap-env/bin/dasbootstrap"]
CMD []

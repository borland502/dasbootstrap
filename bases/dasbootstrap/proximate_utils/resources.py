...
class Resources:
    ...
    def get_vm(self, vmid, ignore_missing=False):
        ...
        try:
            ...
            vms = None
            self.log.error(
                msg=f"Unable to retrieve list of VMs filtered by vmid {vmid}: {e}"
            )
        if vms:
            return vms[0]
        else:
    ...
    def get_storages(self, type):
        ...
        try:
            ...
            self.log.error(
                msg=f"Unable to retrieve storages information with type {type}: {e}"
            )
    ...
    def get_storage_content(self, node, storage, content=None, vmid=None):
        try:
            ...
            self.log.error(
                msg=f"Unable to list content on {node}, {storage} for {content} and {vmid}: {e}"
            )

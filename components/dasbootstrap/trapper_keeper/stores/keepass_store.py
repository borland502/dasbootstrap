...
def _validate_ref(ref: str) -> None:
  ...
  if ref.removeprefix(REF_PREFIX).count(REF_SEP) < 1:
    raise ValueError(f'Invalid ref: {ref}, at least 1 separator {REF_SEP!r} expected', )
  if ref.removeprefix(REF_PREFIX).count(REF_SEP2) != 1:
    raise ValueError(f'Invalid ref: {ref}, exactly 1 separator {REF_SEP2!r} expected', )
...
class KeepassStore(PyKeePass):
  ...
  def __init__(self, kp_fp: Path, kp_token: Path, kp_key: Path | None = None):
    super().__init__(filename=kp_fp, password=kp_token.read_text(encoding="utf-8"), keyfile=kp_key)
  ...
  def write_env(self, entry_path: Sequence[str], env: dict[str, str], create_if_not_exists: bool = True, ) -> None:
    entry = self.find_entries(path=entry_path)
    if entry is None:

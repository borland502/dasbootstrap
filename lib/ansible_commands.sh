...
function _add_to_inventory(){
  ...
  ansible localhost -m "${ABA_Host}" -a 'hostname={{ pve_lxc_net_interfaces[0].ip4 }} ansible_ssh_host={{ pve_hostname }} ansible_ssh_port=22' -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" -e "${SYS_PYTHON}" || exit 5
  ...
  ansible localhost -m "${ABL_IN_File}" -a 'dest=/etc/hosts regexp=.*{{ pve_lxc_net_interfaces[0].ip4 }} line="{{ pve_lxc_net_interfaces[0].ip4 }} {{ pve_hostname }}.{{ pve_lxc_searchdomain }} {{ pve_hostname }}" state=present' -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" -e "${SYS_PYTHON}" --become || exit 5
}
...
function _setup_or_upgrade_lxc(){
  ...
  ansible localhost -m "${ABI_ROLE}" -a name=technohouser.proxmox.create_lxc -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" -e @"${A_INVENTORY}/group_vars/all.yaml" -e @"${A_INVENTORY}/group_vars/proxmox_all_lxc.yaml" -e "${SYS_PYTHON}" || exit 5
  ...
  ansible "${APP_NAME}" -m "${ABSetup}" --user root -e "${SYS_PYTHON}" || exit 5
...
function destroy_lxc_cmd(){
  ...
  ansible localhost -m "${ABL_IN_File}" -a 'dest=/etc/hosts regexp=.*{{ pve_lxc_net_interfaces[0].ip4 }} line="{{ pve_lxc_net_interfaces[0].ip4 }} {{ pve_hostname }}.{{ pve_lxc_searchdomain }} {{ pve_hostname }}" state=absent' -e @"${A_INVENTORY}/host_vars/${APP_NAME}.yaml" -e "${SYS_PYTHON}" --become || exit 5
  _dump_inventory
}

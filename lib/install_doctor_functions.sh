...
# @description Ensure "${LROOT}/lib/logger.sh" is present
function ensure_logdir(){
    if [ ! -f "${LROOT}/logger.sh" ]; then
        mkdir -p .config
        if type curl &> /dev/null; then
            curl -sSL https://gitlab.com/megabyte-labs/common/shared/-/raw/master/common/lib/log > "${LROOT}/logger.sh"
        fi
    fi
    
    chmod +x "${LROOT}/logger.sh"
}
                    ...
                    if type sudo &> /dev/null && sudo -n true; then
                        ...
                        if [ -n "$BREW_EXIT_CODE" ]; then
                            if command -v brew > /dev/null; then
                                "${LROOT}/logger.sh" warn "Homebrew was installed but part of the installation failed. Retrying again after changing a few things.."
